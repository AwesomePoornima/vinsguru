package com.pages.flightreservation;

import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.support.FindBy;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.pages.vendorportal.AbstractPage;



public class FlightConfirmation extends AbstractPage{
	
	private static final Logger log=LoggerFactory.getLogger(FlightConfirmation.class);
	
	@FindBy(css="#flights-confirmation-section .card-body .row:nth-child(1) .col:nth-child(2)")
	private WebElement flightConfirmNum;
	
	@FindBy(css="#flights-confirmation-section .card-body .row:nth-child(3) .col:nth-child(2)")
	private WebElement totalPrice;

	public FlightConfirmation(WebDriver driver) {
		super(driver);
		

	}

	@Override
	public boolean isAt() {
		this.wait.until(ExpectedConditions.visibilityOf(flightConfirmNum));
		return this.flightConfirmNum.isDisplayed();
	}
	
	
	public String fconfirm() {
		String flightnum=this.flightConfirmNum.getText();
		String Price=this.totalPrice.getText();
		log.info("Flight confirmation :{}",flightnum);
		log.info("Total Price :{}",Price);
		return Price;
	}

	

}
